        ...
        async.series([
            ...
            _.bind(function(next){
               ...
               /* this.proxy_server = http_proxy.createProxyServer({
                ...
                var fwd_route =  _.bind(function (req, res, next) {
                ...
                if(_.isString(app.app_data.route.from.alias)){
                    var aliases = [app.app_data.route.from.alias];
                }else{
                    var aliases = app.app_data.route.from.alias;
                }
                for(var i in aliases){
                    this.express_app.use(vhost(aliases[i], fwd_route)); // Serves top level domain via Main server app'
                }
                next();
            }, this),
            callback
